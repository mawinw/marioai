...
public class AStarSimulatorK 
{
    ...
    public float lastEnemyType = -1;
    public float lastEnemyX = -1;
    public float lastEnemyY = -1;
	...
	private class SearchNode
	{
		...
		public float nearestEnemyPosX = -1;
		public float nearestEnemyPosY = -1;
		public float enemyDistThreshold = 10000;
		public float bestDistanceToEnemy = 0;
    ...
    private void search(long startTime)
    {
    	...
    	//mawinw: find nearest enemy
    	float[] e = findNearestEnemy(current);
    	lastEnemyType = e[0];
    	lastEnemyX = e[1];
    	lastEnemyY = e[2];
    	float enemyDistThreshold = e[3];
    	current.nearestEnemyPosX = lastEnemyX;
    	current.nearestEnemyPosY = lastEnemyY;
    	boolean enemyInObservation = false;
    	if(lastEnemyX > -1) {
	    	if (levelScene.verbose > 4) {
	    		System.out.println("the enemy is on observation screen before search");
	    	}
    			...
    			//&& (System.currentTimeMillis() - startTime < Math.min(200,timeBudget/2))) <- this makes the game a bit more jerky, but allows a deeper search in tough situations
    	{
    		...
    		//mawinw: calculate nearest enemy
    		//////////////////////////////////////////////////////////////////////////////////////////

        	e = findNearestEnemy(current);
        	lastEnemyType = e[0];
        	lastEnemyX = e[1];
        	lastEnemyY = e[2];
        	
        	boolean enemyInNode = lastEnemyX > -1 ? true : false;
        	if(enemyInNode) { 
        		//enemy found, update nearest enemy position
        		float dist = utility.calculateDistance(current.parentPos.nearestEnemyPosX, current.parentPos.nearestEnemyPosY,current.sceneSnapshot.mario.x,current.sceneSnapshot.mario.y);
    	    	if (levelScene.verbose > 4) {
    	    		System.out.print("enemy dist :"+dist);
    	    	}
    	    	if(utility.calculateDistance(current.parentPos.nearestEnemyPosX, current.parentPos.nearestEnemyPosY,
    	    								 lastEnemyX						   , lastEnemyX)
    	    								> 10) {
		    		//mawinw: new enemy is selected, meaning mario STOMPED them, or mario run past them and new enemy is closer

						current.bestDistanceToEnemy = 0;
			        	dist = utility.calculateDistance(lastEnemyX , lastEnemyY, current.sceneSnapshot.mario.x,current.sceneSnapshot.mario.y);

			    		if (levelScene.verbose > 4) System.out.println("NEW ENEMY IS A TARGET");
			    		if (levelScene.verbose > 4) System.out.println("NEW enemy dist :"+ dist);
		    	}
	        	current.nearestEnemyPosX = lastEnemyX;
	        	current.nearestEnemyPosY = lastEnemyY;
    	    	...
    	    	if (levelScene.verbose > 3) {
    	    		System.out.println("remaining time + ENEMY distance: "+realRemainingTime);
    	    	}
    			if(enemyDistThreshold - dist > current.parentPos.bestDistanceToEnemy ) {

        	    	if (levelScene.verbose > 3) {
        	    		System.out.println("-----dist threshold"+enemyDistThreshold+" - dist"+dist+" > "+current.parentPos.bestDistanceToCoin+""
    						+ "\n make current node (GOOD)?-----");
        	    		System.out.print("dist threshold:");
        	    		System.out.print(enemyDistThreshold);
        	    		System.out.print("dist:");
        	    		System.out.print(dist);
        	    		System.out.print("bestDistanceToCoin:");
        	    		System.out.println(enemyDistThreshold - dist);
        	    	}
    				current.bestDistanceToEnemy = enemyDistThreshold - dist;
    				//distThreshold = dist;
    				//currentGood = true;
    		...
    		else if (realRemainingTime - current.remainingTimeEstimated >= enemyDistThreshold && enemyInNode) 
    			//mawinw: if there are coin: use this
    		{

    	    	if (levelScene.verbose > 4) {
    	    		System.out.println("case: [ENEMY] on original screen");
    			...
    			//mawinw: if there are coin: use this
    		{
    	    	if (levelScene.verbose > 4) {
    				...
    				System.out.print(distThreshold);
    				System.out.print(" \nthe node is going [CLOSER][CLOSER][CLOSER] to the [ENEMY]: ");
    				System.out.println(realRemainingTime - current.remainingTimeEstimated);
    	    	}
    ...
    private float[] findNearestEnemy(SearchNode node) {
    	float[] ret = node.sceneSnapshot.findNearestEmemy(node.sceneSnapshot.mario.x,node.sceneSnapshot.mario.y);
    	return ret;
    }
    ...
    private ArrayList<boolean[]> extractPlan()
    {
    	...
    	if (bestPosition == null)
    	{
    		...
    		for (int i = 0; i < 2; i++)
    		{
    			...
    			actions.add(createAction(false, false, false, true, false)); //j
    			actions.add(createAction(false, false, false, true, true));  //js
    	    	
    	    	// run right
    			actions.add(createAction(false, true, false, true, true));  //rj
    			actions.add(createAction(false, true, false, true, false)); //rjs
    	 	
    	    	// run left
    			actions.add(createAction(true, false, false, true, false)); //lj
    			actions.add(createAction(true, false, false, true, true));  //ljs
    	...
    	while (current.parentPos != null)
    	{
    		for (int i = 0; i < current.repetitions; i++) 
    		{
        		...
        		if (levelScene.verbose > 2) 
        			System.out.print("added"/* + printAction(current.action)*/);
    		}
    		if (levelScene.verbose > 2) 
    			System.out.print("[" 
